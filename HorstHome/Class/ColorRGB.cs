...
namespace HorstHome
{
    public struct ColorRGB
    {
        ...
        public static double[] RgBtoHsl(Color color)
        {
            ...
            // hue
            if (SystemMath.Abs(max - min) < ColorRGB.tolerance)
                h = 0D; // undefined
            else if ((SystemMath.Abs(max - r) < ColorRGB.tolerance)
                    && (g >= b))
                h = (60D * (g - b)) / (max - min);
            else if ((SystemMath.Abs(max - r) < ColorRGB.tolerance)
                    && (g < b))
                h = ((60D * (g - b)) / (max - min)) + 360D;
            else if (SystemMath.Abs(max - g) < ColorRGB.tolerance)
                h = ((60D * (b - r)) / (max - min)) + 120D;
            else if (SystemMath.Abs(max - b) < ColorRGB.tolerance)
            ...
            // saturation
            if ((SystemMath.Abs(l) < ColorRGB.tolerance)
                    || (SystemMath.Abs(max - min) < ColorRGB.tolerance))
                s = 0D;
            else if ((0D < l)
                    && (l <= .5D))
                s = (max - min) / (max + min);
        ...
        public static Color HsLtoRgb(double h, double s, double l, int a = 255)
        {
            ...
            if (SystemMath.Abs(s) < ColorRGB.tolerance)
            {
        ...
        public static double[] RgBtoHsb(Color color)
        {
            ...
            double h = 0D;
            if ((SystemMath.Abs(max - r) < ColorRGB.tolerance)
                    && (g >= b))
                h = (60D * (g - b)) / (max - min);
            else if ((SystemMath.Abs(max - r) < ColorRGB.tolerance)
                    && (g < b))
            ...
            double s = SystemMath.Abs(max) < ColorRGB.tolerance
                    ? 0D
                    : 1D - (min / max);
                ...
                byte? outputAlpha = null)
        {
            double[] hsl = colorTransformMode == ColorTransformMode.Hsl
                    ? ColorRGB.RgBtoHsl(color)
                    : ColorRGB.RgBtoHsb(color);
            if ((SystemMath.Abs(hsl[2]) < ColorRGB.tolerance)
            ...
            return colorTransformMode == ColorTransformMode.Hsl
                    ? ColorRGB.HsLtoRgb(hsl[0], hsl[1], hsl[2], outputAlpha ?? color.A)
                    : ColorRGB.HsBtoRgb(hsl[0], hsl[1], hsl[2], outputAlpha ?? color.A);
        }
                ...
                byte? outputAlpha = null)
        {
            double[] hsl = colorTransformMode == ColorTransformMode.Hsl
                    ? ColorRGB.RgBtoHsl(color)
                    : ColorRGB.RgBtoHsb(color);
            if ((SystemMath.Abs(hsl[1]) < ColorRGB.tolerance)
